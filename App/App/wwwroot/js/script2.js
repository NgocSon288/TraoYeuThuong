...
const time = 3000 // thời gian in một dòng
const delayRowItem = 1000 // Thời gian nghỉ khi in xong 1 dòng
const delayPage = 5000
...
const page2Event = function() {
    ...
    const audioVal = document.getElementById('poem-audio')
    const path = window.location.origin + `/Upload/` + audioVal.value
    const btn = document.querySelector('.btn-first')
    const audioFileUrl = path
...
setTimeout(() => { continueSection.classList.remove('d-none') }, (time + delayRowItem) * 13 + delayPage)